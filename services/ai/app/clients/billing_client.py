...
class BillingClient:
    ...
    async def reserve(self, user_id: str, request_id: str, cost: int = 1):
        return await self._post_form("/internal/usage/reserve", {"user_id": user_id, "request_id": request_id, "cost": str(cost)})

    async def cancel(self, user_id: str, request_id: str):
        return await self._post_form("/internal/usage/cancel", {"user_id": user_id, "request_id": request_id})

    async def confirm(self, user_id: str, request_id: str, task_id: str):
        return await self._post_form("/internal/usage/confirm", {"user_id": user_id, "request_id": request_id, "task_id": task_id})
